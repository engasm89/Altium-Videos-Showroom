...
export const LearningPathView: React.FC<LearningPathViewProps> = ({
  ...
  const primaryPaths = LEARNING_PATHS.filter((p) => p.featured);
  const morePaths = LEARNING_PATHS.filter((p) => !p.featured);
        ...
        >
          <div className="flex items-start space-x-4">
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs sm:text-sm text-slate-300 line-clamp-2">{path.headline}</p>
              <div className="flex flex-wrap items-center gap-4 text-xs font-mono text-slate-400 pt-1">
                ...
                <span className="flex items-center space-x-1">
                  ...
                  <span>{new Set(path.modules.flatMap((m) => m.tutorialIds)).size} Tutorials</span>
                </span>
                <span>•</span>
          ...
          <div className="flex items-center justify-between md:justify-end gap-4 shrink-0 pt-3 md:pt-0 border-t md:border-t-0 border-slate-800">
            <div className="text-right">
              <div className="text-xs font-mono text-slate-300">
                Progress:{' '}
                <span className="text-emerald-400 font-bold">{stats.percentage}%</span> ({stats.completed}/
                {stats.total})
              </div>
          ...
          <div className="p-5 sm:p-6 bg-slate-950/70 border-t border-slate-800/80 space-y-6">
            <div className="p-4 bg-slate-900/90 rounded-xl border border-slate-800">
              <div className="space-y-1">
                ...
                <p className="text-sm text-slate-200 leading-relaxed">{path.outcome}</p>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {path.skillsAcquired.map((skill, idx) => (
                  <span
                    key={idx}
                    className="text-xs bg-slate-900 text-cyan-300 border border-slate-800 px-3 py-1 rounded-lg font-mono flex items-center space-x-1"
                  >
                            ...
                            <div className="flex items-center space-x-3">
                              <div
                                className={`p-1.5 rounded ${
                                  isDone ? 'text-emerald-400 bg-emerald-950' : 'text-blue-400 bg-slate-900'
                                }`}
                              >
                                <Play className="w-3.5 h-3.5 fill-current" />
                              </div>
                              <div>
                                <div className="text-xs font-medium hover:text-blue-300 transition-colors">
                                  {tut.title}
                                </div>
                                <div className="text-[10px] text-slate-400 font-mono">
                                  {tut.durationFormatted} • {tut.product}
                                </div>
                              </div>
                            </div>
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8 text-slate-100">
      <div className="space-y-3">
        <div className="inline-flex items-center space-x-2 text-xs font-mono bg-blue-950 text-blue-300 border border-blue-800 px-3 py-1 rounded-full">
          ...
          <span>Altium Designer &amp; Develop outcome paths</span>
        </div>
        <h2 className="text-2xl sm:text-4xl font-extrabold text-white tracking-tight">
          {primaryPaths.length} Primary Learning Paths
        </h2>
        <p className="text-slate-300 text-sm sm:text-base max-w-3xl leading-relaxed">
          Primary UI highlights the strongest Designer and Develop curricula. Additional specialty paths stay available
          below — this is not a dozens-of-tracks LMS homepage.
        </p>
      </div>

      <div className="space-y-6">{primaryPaths.map(renderPathCard)}</div>

      {morePaths.length > 0 && (
        <div className="space-y-4 pt-4 border-t border-slate-800">
          <div className="space-y-1">
            <h3 className="text-sm font-bold text-slate-200 font-mono uppercase tracking-wider">
              More paths ({morePaths.length})
            </h3>
            <p className="text-xs text-slate-500">
              Secondary / specialty tracks kept for depth — not the partner-facing hero set.
            </p>
          </div>
          <div className="space-y-4">{morePaths.map(renderPathCard)}</div>
        </div>
      )}
    </div>
  );
