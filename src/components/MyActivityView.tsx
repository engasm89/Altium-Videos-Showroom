...
/**
 * My Activity = YOUR browser only (localStorage).
 * Not site-wide analytics — see /insights for PostHog/GA4.
 */
export const MyActivityView: React.FC<MyActivityViewProps> = ({
  ...
  const handleExportLocalReport = () => {
    const reportData = {
      platform: 'EET Electronics Product Development Library',
      url: 'learn.eduengteam.com',
      generatedDate: new Date().toISOString(),
      note: 'Metrics below are YOUR browser only (localStorage). They are not site-wide analytics. For aggregates see PostHog/GA4 via /insights.',
      catalogMetrics: {
    ...
    a.href = url;
    a.download = `EET_My_Activity_${new Date().toISOString().slice(0, 10)}.json`;
    a.click();
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8 text-slate-100">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800 pb-6">
        <div className="space-y-2">
          <div className="inline-flex items-center space-x-2 text-xs font-mono bg-cyan-950 text-cyan-300 border border-cyan-800 px-3 py-1 rounded-full">
            ...
            <span>YOUR browser only — not site-wide analytics</span>
          </div>
          <h2 className="text-2xl sm:text-4xl font-extrabold text-white tracking-tight">My Activity</h2>
          <p className="text-slate-300 text-sm max-w-2xl leading-relaxed">
            Completions, bookmarks, outbound clicks, and searches stored in <strong className="text-white font-semibold">this browser&apos;s
            localStorage</strong>. Clearing site data resets these numbers. Site-wide aggregates require PostHog/GA4
            (see Insights) — they are never invented here.
          </p>
        </div>

        <button
          type="button"
          onClick={handleExportLocalReport}
        ...
        >
          ...
          <span>Export My Activity JSON</span>
        </button>
      </div>
      ...
      {reportExported && (
        <div className="p-4 bg-emerald-950/80 border border-emerald-800 rounded-xl text-emerald-300 text-xs font-mono flex items-center space-x-2 animate-fadeIn">
          ...
          <span>Local activity JSON exported. Share only what this browser actually recorded.</span>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-slate-900 border border-slate-800 p-5 rounded-xl space-y-2">
          <div className="flex items-center justify-between text-xs font-mono text-slate-400">
            <span>Your Altium CTA clicks</span>
            <ExternalLink className="w-4 h-4 text-amber-400" />
          </div>
          <div className="text-3xl font-extrabold text-amber-400 font-mono">{outboundLogs.length}</div>
          <div className="text-[11px] text-slate-400">
        ...
        <div className="bg-slate-900 border border-slate-800 p-5 rounded-xl space-y-2">
          <div className="flex items-center justify-between text-xs font-mono text-slate-400">
            <span>Your search gaps</span>
            <Search className="w-4 h-4 text-rose-400" />
          </div>
          <div className="text-3xl font-extrabold text-rose-400 font-mono">{zeroResultSearches.length}</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl space-y-4 shadow-xl">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-bold text-white flex items-center space-x-2">
              <ExternalLink className="w-4 h-4 text-amber-400" />
              <span>Your outbound clicks</span>
            </h3>
          ...
          <div className="space-y-2 max-h-60 overflow-y-auto pr-1">
            {outboundLogs.length === 0 ? (
              <p className="text-xs text-slate-500 font-mono p-3 bg-slate-950 rounded-xl border border-slate-800">
                No outbound clicks recorded yet in this browser.
              </p>
            ) : (
        ...
        <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl space-y-4 shadow-xl">
          ...
          <p className="text-xs text-slate-300">
            Zero-result searches highlight topics you looked for that are not yet in the enriched catalog.
          </p>

          <div className="space-y-2 max-h-60 overflow-y-auto pr-1">
            {searchLogs.length === 0 ? (
              <p className="text-xs text-slate-500 font-mono p-3 bg-slate-950 rounded-xl border border-slate-800">
                No searches logged yet in this browser.
              </p>
            ) : (
      ...
      <div className="p-6 bg-gradient-to-r from-slate-900 via-blue-950 to-slate-900 border border-blue-800 rounded-2xl space-y-4 shadow-2xl">
        ...
        <p className="text-xs text-slate-300 leading-relaxed">
          ...
          {catalogCounts.enriched} hand-enriched overlays, {LEARNING_PATHS.length} outcome paths,{' '}
          {HARDWARE_PROJECTS.length} project hubs, and verifiable local engagement signals — not invented visitor or
          country metrics. Site-wide funnels belong in PostHog/GA4 after keys are enabled.
        </p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs pt-2">
          <div className="p-3 bg-slate-950/80 rounded-xl border border-slate-800">
            ...
            <p className="text-[11px] text-slate-400">
              Trademark-safe EET branding, conspicuous independent disclaimers, robust taxonomy.
            </p>
          </div>
          <div className="p-3 bg-slate-950/80 rounded-xl border border-slate-800">
            <div className="font-semibold text-white mb-1">2. Honest scale</div>
            <p className="text-[11px] text-slate-400">
              {catalogCounts.total} catalog rows imported; {catalogCounts.playable} playable embeds;{' '}
              {catalogCounts.enriched} hand-enriched.
            </p>
          </div>
          <div className="p-3 bg-slate-950/80 rounded-xl border border-slate-800">
